...
with registro_tab:
    ...
    with st.container(border = True):
        ...
        pwd = st.text_input("Contraseña (Mínimo 8 caracteres)", "", type = "password")#, key = 'pwd1')
        if len(pwd) < 8:
            st.error("Contraseña inválida: introduce mínimo 8 caracteres")
        nombre_real = st.text_input("Nombre", "")#, key = 'realname1')
        if len(nombre_real) == 0:
            st.error("Introduce tu nombre")
        apellido = st.text_input("Apellido (sólo uno)", "")#, key = 'lastname1')
        if len(apellido) == 0:
            st.error("Introduce tu apellido")
        edad = st.number_input("Edad (15+)",15,100, value = None)#, key = 'age1')
        if edad == None:
            st.error("Introduce tu edad")
        dispositivoID = st.text_input("Introduce el ID de tu HealthMet (ej: HEALTHMET_001)", "")
        ...
        if df.iloc[0]['Total'] > 0:
            ...
            dispositivoID = ""
        dispositivo = st.text_input("Escoge un nombre para tu casco", "HealthMet")#, key = 'device1')
